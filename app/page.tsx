...
export default function DashboardPage() {
  ...
  useRealtime((msg) => {
    if (msg.type === "EMAIL_PARSED" || msg.type === "RISK_ALERT_CREATED") {
      fetchAlerts().then(setAlerts);
      fetchKPIs().then(setKpis);
      fetchPOs().then(setPos);
      toast.info("⚡ Live Sync: Supplier Delay Ingested", {
        description: msg.data?.summary || "Supplier delay received. Stockout alerts and replenishment updated.",
        duration: 5000,
      });
    } else if (msg.type === "PO_CREATED" || msg.type === "PO_UPDATED") {
      fetchPOs().then(setPos);
      fetchKPIs().then(setKpis);
      toast.success("⚡ Live Sync: Purchase Order Updated", {
        description: `PO ${msg.data?.id || msg.data?.po_id || ""} updated (${msg.data?.status || "active"}).`,
        duration: 4000,
      });
    } else if (msg.type === "SCENARIO_RUN") {
      toast.info("⚡ Live Sync: What-If Simulation Completed", {
        description: `New stockouts: ${msg.data?.newStockoutCount ?? 0} | Financial impact: $${Number(msg.data?.costImpact ?? 0).toLocaleString()}`,
        duration: 4000,
      });
    }
  });
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2 shrink-0">
          <label className="inline-flex items-center gap-2 rounded-lg border border-blue-500/40 bg-blue-500/10 hover:bg-blue-500/20 text-blue-600 dark:text-blue-400 text-xs font-semibold px-3 py-2 transition-colors cursor-pointer shadow-sm">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-lg border border-primary/40 bg-primary/10 hover:bg-primary/20 text-primary text-xs font-semibold px-3 py-2 transition-colors shadow-sm"
          >
