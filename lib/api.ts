...
export async function fetchAlerts(): Promise<Alert[]> {
  try {
    const res = await fetch(`http://127.0.0.1:8000/alerts?t=${Date.now()}`);
    if (res.ok) {
      const payload = await res.json();
      return (Array.isArray(payload) ? payload : []).map((alert) => ({
        id: String(alert.id ?? ""),
        sku: String(alert.sku ?? alert.sku_id ?? ""),
        skuName: String(alert.skuName ?? alert.sku_name ?? alert.sku ?? ""),
        riskLevel: alert.riskLevel ?? alert.risk_level ?? "low",
        daysUntilStockout: alert.daysUntilStockout ?? alert.days_until_stockout ?? null,
        currentStock: Number(alert.currentStock ?? alert.current_stock ?? 0),
        forecastedDemand: Number(alert.forecastedDemand ?? alert.forecasted_demand ?? alert.predicted_demand ?? 0),
