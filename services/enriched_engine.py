...
Feature map
-----------
  Feature 1 ΓÇö Emergency Decision Engine   : make_procurement_decision()
  Feature 2 ΓÇö Supplier Trust Score        : update_supplier_trust_scores()
  Feature 3 ΓÇö Inventory Confidence Score  : compute_inventory_confidence()
  Feature 4 ΓÇö Predictive Depletion Alert  : check_depletion()
  Feature 5 ΓÇö Idempotent Recalibration    : apply_feedback_safely()
"""
...
# ===========================================================================
#
# "Site" in this codebase maps to supplier_id ΓÇö anomalies (price deviations,
...
#   anomaly_rate   (weight 0.6) ΓÇö fraction of rows flagged is_anomaly==True
#   weekend_rate   (weight 0.2) ΓÇö fraction placed on Sat/Sun (day-of-week 5/6)
#   amount_factor  (weight 0.2) ΓÇö avg(Price ├ù Units Ordered) / AMOUNT_CEILING
#
# Amount ceiling calibration:
#   Actual data: mean order amount Γëê 6 071, 95th pct Γëê 14 629, max Γëê 19 978.
#   Reference code used 50 000 ΓÇö that would compress every supplier to Γëê0.12,
#   making the factor meaningless.  We use 15 000 (Γëê 95th pct) so high-value
#   suppliers actually register on the scale.
#
# Traffic-light thresholds (configurable at call time):
#   green  : risk_score < 0.30
#   yellow : 0.30 Γëñ risk_score < 0.60
#   red    : risk_score ΓëÑ 0.60
#
# Minimum sample guard: suppliers with < 10 rows return risk_score=None.
# ===========================================================================

_AMOUNT_CEILING = 15_000.0   # Γëê 95th percentile of (Price ├ù Units Ordered)
...
def predict_supplier_risk(rows_for_supplier: pd.DataFrame) -> dict:
    ...
    # Signal 1 ΓÇö anomaly rate: is_anomaly is a native bool column
    anomaly_rate = float(rows_for_supplier["is_anomaly"].mean())

    # Signal 2 ΓÇö weekend rate: derived from Date (dayofweek 5=Sat, 6=Sun)
    weekend_mask = rows_for_supplier["Date"].dt.dayofweek >= 5
    weekend_rate = float(weekend_mask.mean())

    # Signal 3 ΓÇö order amount factor: Price ├ù Units Ordered, capped at ceiling
