"""
services/supplier_outreach.py ΓÇö Corrected supplier outreach module.
...
Key improvements vs. the old flat-threshold version:
  - is_low_stock()           ΓÇö relative check (stock vs. reorder_level), not a hardcoded number
  - maybe_refresh_supplier_quote() ΓÇö gate on decision, only fires when procurement is actually needed
  - Price anchored to supplier['last_price'] * drift (0.92ΓÇô1.08), not random.uniform(10, 50)
...
def _do_simulated_call(sku_id: str, supplier: dict) -> dict:
    ...
    sensible range ($50ΓÇô$300) only on the very first call when no history exists.
    ...
    if last_price and last_price > 0:
        # ┬▒8% drift from last known price ΓÇö stays consistent with demo data
    ...
    print(
        f"[Supplier Outreach] Call to '{supplier['name']}' for SKU {sku_id} ΓåÆ "
...
# ---------------------------------------------------------------
# Public API ΓÇö called by run_agent()
# ---------------------------------------------------------------
...
def maybe_refresh_supplier_quote(
    ...
    if decision not in PROCUREMENT_DECISIONS:
        # Internal transfer chosen ΓÇö no supplier call needed
        return None

    if not is_low_stock(item, supplier):
        # Stock is healthy relative to reorder level ΓÇö skip unnecessary call
        return None
...
def real_supplier_call(
    ...
    while waited < max_wait_seconds:
        ...
        print(f"[Vapi] Poll {waited}s ΓÇö status={call_status}")
        ...
        if call_status == "ended":
            ...
            if extracted and extracted.get("price") is not None:
                ...
                print(
                    f"[Vapi] Call complete for SKU {sku_id} ΓåÆ "
                    f"price={quote['price']} lead={quote['lead_time_days']}d "
                    f"avail={quote['availability']}"
                )
                return quote
            else:
                print("[Vapi] Call ended but structured data missing ΓÇö falling back to simulation")
                break
