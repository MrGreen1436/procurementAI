...
services/alerts.py
──────────────────────────────────────────────────────────────────────────────
Multi-Channel Alerts — Slack integration.

Design principles
-----------------
• Graceful no-op: if SLACK_WEBHOOK_URL is not set in the environment the
  function returns immediately without raising, so the caller is never
  affected by an unconfigured channel.
• Zero new dependencies: uses httpx which is already a FastAPI transitive
  dependency and is confirmed available in this environment.
• Additive only: nothing in this module imports from or modifies any other
  service file.

Anomaly record structure (from enriched_engine.ANOMALY_RECORDS):
  {
    "store_id":         str   — e.g. "S001"
    "product_id":       str   — e.g. "P0001"
    "date":             str   — ISO date, e.g. "2023-06-15"
    "supplier_id":      str   — e.g. "SUP-003"
    "anomaly_reason":   str   — one of:
...
Severity mapping (derived from anomaly_reason, no new field required):
  "Both order quantity and price are anomalous" → CRITICAL
  "Order quantity far exceeds demand forecast"  → HIGH
  "Price significantly deviates from ..."       → HIGH
  (anything else / unknown)                     → MEDIUM
"""
...
# ---------------------------------------------------------------------------
# Severity derivation — based on existing anomaly_reason values in the data
# ---------------------------------------------------------------------------
...
def _build_slack_message(anomaly_record: dict, severity: str) -> dict:
    ...
    # Slack Block Kit payload — renders nicely in Slack; falls back gracefully
    return {
        "text": f"{severity_emoji} [{severity}] Procurement anomaly flagged — {store_id} / {product_id}",
        "blocks": [
            {
                "type": "header",
                "text": {
                    "type": "plain_text",
                    "text": f"{severity_emoji} Procurement Anomaly — {severity}",
                    "emoji": True,
                },
            },
            {
...
def send_slack_alert(
    ...
    Parameters
    ----------
    anomaly_record : dict — one entry from enriched_engine.ANOMALY_RECORDS
    ...
    Guarantees
    ----------
    • Never raises — all exceptions are caught and returned as {"status": "failed"}.
    • If SLACK_WEBHOOK_URL env var is absent, returns {"status": "skipped"} immediately.
    • If severity < min_severity, returns {"status": "filtered"} without hitting the network.
    """
    webhook_url = os.environ.get("SLACK_WEBHOOK_URL")

    # ── Graceful no-op if unconfigured ───────────────────────────────────────
    if not webhook_url:
        logger.debug("[Alerts] SLACK_WEBHOOK_URL not set — skipping Slack alert.")
    ...
    if _severity_rank.get(severity, 0) < _severity_rank.get(min_severity, 1):
        logger.debug(
            "[Alerts] Anomaly severity %s < min_severity %s — filtered out.",
    ...
    try:
        ...
        logger.info(
            "[Alerts] Slack alert sent — store=%s product=%s severity=%s reason=%s",
